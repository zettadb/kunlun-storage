...
static char *opt_begin_from = nullptr;
static MYSQL *mysql = nullptr;
static char current_db[] = "test";
static unsigned int test_count = 0;
static unsigned int opt_count = 0;
static unsigned int opt_count_read = 0;
static unsigned int iter_count = 0;
//static bool have_innodb = true;
...
static MYSQL *mysql_client_init(MYSQL *con) {
  ...
  uint ssl_mode = SSL_MODE_DISABLED;// dzw: SSL_MODE_REQUIRED
#if defined(_WIN32)
...
static MYSQL *client_connect(ulong flag, uint protocol, bool auto_reconnect) {
  ...
  strxmov(query, "CREATE DATABASE ", current_db, NullS);// dzw: IF NOT EXISTS 
...
static struct my_option client_test_long_options[] = {
    {"begin", 'B', "Run tests beginning from this test case.", &opt_begin_from, &opt_begin_from, nullptr,
    ...
    {"default_auth", 0, "Default authentication client-side plugin to use.",
     ...
     0, nullptr, 0, nullptr},
    {"show-tests-bug-wl", 'y', "Show all bug&wl tests' names", nullptr, nullptr, nullptr,
     GET_NO_ARG, NO_ARG, 0, 0, 0, nullptr, 0, nullptr},
    {"bug_wl", 'z', "Run test cases for bugs and WLs", nullptr, nullptr,
...
struct my_tests_st *get_my_bug_wl_tests();
...
static bool get_one_option(int optid,
  ...
  switch (optid) {
    ...
    case 'y': {
      ...
      printf("All possible test names:\n\n");
      for (fptr = get_my_bug_wl_tests(); fptr->name; fptr++) printf("%s\n", fptr->name);
      exit(0);
      break;
    }
	case 'z': my_testlist = get_my_bug_wl_tests(); break;
    case '?':
    case 'I': /* Info */
      usage();
      exit(0);
...
int main(int argc, char **argv) {
  ...
  for (iter_count = 1; iter_count <= opt_count; iter_count++) {
    ...
    if (!tests_to_run) {
	  ...
	  {
		if (!started && (!opt_begin_from || (opt_begin_from && strcmp(opt_begin_from, fptr->name) == 0)))
		  started = true;
	    if (started)
		  (*fptr->function)();
	  }
