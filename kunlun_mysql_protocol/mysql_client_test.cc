...
static void test_prepare_insert_update() {
  ...
  const char *testcase[] = {
      ...
      //"INSERT INTO t1 VALUES (5,6,30), (7,4,40), (8,9,60) ON DUPLICATE KEY UPDATE "
      //"c=c+100",
      "SELECT * FROM t1",
      //"INSERT t1 SET a=5 ON DUPLICATE KEY UPDATE b=0",
      "INSERT INTO t1 (a) values(0)",
      "SELECT * FROM t1",
      //"INSERT INTO t1 VALUES (2,1,11), (7,4,40) ON DUPLICATE KEY UPDATE "
      //"c=c+VALUES(a)",
      nullptr};
...
static void test_wl4435_3() {
  ...
  */
  ...
  WL4435_TEST("TINYINT", "127", MYSQL_TYPE_SHORT, MYSQL_TYPE_SHORT, char, ,
              ("  - TINYINT / char / MYSQL_TYPE_SHORT:\t\t\t %d", (int)pspv),
              pspv == 127);
...
static void test_prepare_field_result() {
  ...
  rc = mysql_query(mysql,
                   ...
                   "char_c char(4), date_c date, extra smallint)");
  myquery(rc);
  ...
  // because of the bug kunlun-server bug#742, we have to set table name and orig table name to NULL to disable the check.
  // also, length isn't the same because for some types of fields kunlun-server store its values differently, so not checking length either.
#if 0
...
#endif
  verify_prepare_field(result, 0, "int_c", "int_c", MYSQL_TYPE_LONG, NULL,
                       NULL, current_db, 0, 0);
  verify_prepare_field(result, 1, "var_c", "var_c", MYSQL_TYPE_VAR_STRING, NULL,
                       NULL, current_db, 0, 0);
  verify_prepare_field(result, 2, "date", "date"/* should be "date_c" but because of #742 */, MYSQL_TYPE_DATE, NULL,
                       NULL, current_db, 0, 0);
  verify_prepare_field(result, 3, "ts_c", "ts_c", MYSQL_TYPE_TIMESTAMP, NULL,
                       NULL, current_db, 0, 0);
  verify_prepare_field(
      result, 4, "char_c", "char_c",
      (mysql_get_server_version(mysql) <= 50000 ? MYSQL_TYPE_VAR_STRING
                                                : MYSQL_TYPE_STRING),
      NULL, NULL, current_db, 0, 0);
...
static void test_prepare() {
  ...
  rc = mysql_query(mysql,
                   "CREATE TABLE my_prepare(col1 smallint, "
  ...
  for (tiny_data = 0; tiny_data < 100; tiny_data++) {
	if (tiny_data == 10) // test both autocommit inserts and explicit txn
  ...
  for (o_tiny_data = 0; o_tiny_data < 100; o_tiny_data++) {
    ...
    if (cmp_double(&double_data, &o_double_data))
      fprintf(stdout, "\n\t double_data(%g) != o_double_data(%g)", double_data, o_double_data);
...
static void test_double_compare() {
  ...
  rc = mysql_query(mysql,
                   "CREATE TABLE test_double_compare(col1 smallint, "
...
static void test_ps_null_param() {
  ...
  size_t len1 = sizeof(in_long);
  in_bind[1].buffer_type = MYSQL_TYPE_LONG;
  in_bind[1].is_null = &in_is_null1;
  in_bind[1].length = &len1;
  in_bind[1].buffer = (void *)&in_long;
  in_is_null1 = true;
  in_long = 1;

  char tmpstr[32] = "abc";
  in_bind[0].buffer_type = MYSQL_TYPE_STRING;
  in_bind[0].is_null = &in_is_null0;
  in_bind[0].length = nullptr;
  in_bind[0].buffer = (void *)tmpstr;
  in_is_null0 = true;

  out_bind[0].buffer_type = MYSQL_TYPE_STRING;
  out_bind[0].is_null = &out_is_null;
  out_bind[0].length = &out_length;
  out_bind[0].buffer = out_str_data;
  out_bind[0].buffer_length = array_elements(out_str_data);

  long intval = 0;
  out_bind[1].buffer_type = MYSQL_TYPE_LONG;
  out_bind[1].is_null = &out_is_null;
  out_bind[1].length = &out_length;
  out_bind[1].buffer = &intval;
  out_bind[1].buffer_length = sizeof(intval);

  /* Execute several queries, all returning NULL in result. */
  int idx = 0;
  for (cur_query = queries; *cur_query; cur_query++, idx++) {
    char query[MAX_TEST_QUERY_LENGTH];
    my_stpcpy(query, *cur_query);
    stmt = mysql_simple_prepare(mysql, query);
    check_stmt(stmt);
	...
	if (idx == 2) in_is_null1 = false;

	MYSQL_BIND *p_bind = (cur_query == queries ? in_bind : in_bind+1);
	MYSQL_BIND *po_bind = (cur_query == queries ? out_bind : out_bind+1);
    rc = mysql_stmt_bind_param(stmt, p_bind);

    check_execute(stmt, rc);
    rc = mysql_stmt_bind_result(stmt, po_bind);
    check_execute(stmt, rc);
    rc = mysql_stmt_execute(stmt);
    check_execute(stmt, rc);
    rc = mysql_stmt_fetch(stmt);
    DIE_UNLESS(rc != MYSQL_NO_DATA);
    DIE_UNLESS(out_is_null || idx >= 2);
    rc = mysql_stmt_fetch(stmt);
    DIE_UNLESS(rc == MYSQL_NO_DATA);
    mysql_stmt_close(stmt);
  }
...
static void test_select_direct() {
  ...
  // autocommit always true for kunlun, can't support the setting for now, it requires handling of implicit txn commit which isn't done yet.
  //rc = mysql_autocommit(mysql, true);
  //myquery(rc);
  ...
  rc = mysql_query(mysql,
                   "CREATE TABLE test_select(id int, id1 smallint, "
                   " id2 float, "
                   " id3 double, "
                   " name varchar(50))");
  myquery(rc);
...
static void test_select_prepare() {
  ...
  rc = mysql_query(mysql,
                   "CREATE TABLE test_select(id smallint, id1 int, "
                   "  id2 float, id3 float, "
                   "  name varchar(50))");
  myquery(rc);
...
static void test_select_show() {
  ...
  rc = mysql_query(mysql,
                   "CREATE TABLE test_show(id int NOT NULL primary "
                   " key, name char(2))");
  myquery(rc);
...
static void test_long_data_str1() {
  ...
  rc = mysql_query(mysql,
                   "CREATE TABLE test_long_data_str(longstr varchar(256), blb "
                   "long varbinary)");
  myquery(rc);
  ...
  sprintf(data, "%d", i * 2);
  // blob values are hex encoded and length() is pushed down to storage node
  // which returns the encoded length. this is OK for memory space allocation
  // because the encoded length is always more than twice the original length.
...
static void test_time_zone() {
  ...
  myquery(mysql_query(mysql, "SET TimeZone = 'GMT';"));
  myquery(mysql_query(mysql, "drop table if exists ttz "));
  myquery(mysql_query(mysql, "drop table if exists tdt "));
  ...
  verify_col_data("ttz", "a", "2011-02-02 15:31:06+00");
...
static void test_bind_result_ext() {
  ...
  rc = mysql_query(mysql,
                   "CREATE TABLE test_bind_result(c1 smallint, "
                   " c2 smallint, "
                   " c3 int, c4 bigint, "
                   " c5 float, c6 double, "
                   " c7 varbinary(10), "
...
static void test_bind_result_ext1() {
  ...
  rc = mysql_query(mysql,
                   "CREATE TABLE test_bind_result(c1 smallint, c2 smallint, \
                                                        c3 int, c4 bigint, \
                                                        c5 float, c6 double, \
                                                        c7 varbinary(10), \
                                                        c8 varchar(10))");
...
static void bind_fetch(int row_count) {
  ...
  while (row_count--) {
    ...
    DIE_UNLESS(length[0] == 2);// in kunlun-server we use int16 for tiny.
    rc += 13;
...
static void test_fetch_date() {
  ...
  rc = mysql_query(mysql,
                   "INSERT INTO test_bind_result VALUES('2002-01-02', \
                                                              ...
                                                              2010, \
                                                              '2010-07-10 17:46:59', \
                                                              '2020-01-02 17:46:59', '1999-12-29 17:46:59')");
  myquery(rc);
  ...
  DIE_UNLESS(strcmp(dt, "2010-07-10 17:46:59") == 0);
  DIE_UNLESS(dt_length == 19);

  DIE_UNLESS(strcmp(ts_4, "2020-01-02 17:46:59") == 0);
  DIE_UNLESS(ts4_length == strlen("2020-01-02 17:46:59"));

  DIE_UNLESS(strcmp(ts_6, "1999-12-29 17:46:59") == 0);
...
static void test_fetch_long() {
  ...
  rc = mysql_query(mysql,
                   "CREATE TABLE test_bind_fetch(c1 int, \
                                                     c2 int, \
                                                     c3 int, \
                                                     c4 int, \
                                                     c5 int, \
...
static void test_fetch_short() {
  ...
  rc = mysql_query(mysql,
                   "CREATE TABLE test_bind_fetch(c1 smallint, \
                                                     c2 smallint, \
                                                     c3 smallint, \
                                                     c4 smallint, \
                                                     c5 smallint, \
...
static void test_fetch_float() {
  ...
  rc = mysql_query(mysql,
                   "CREATE TABLE test_bind_fetch(c1 float, \
                                                     c2 float, \
                                                     c3 float, \
                                                     c4 float, \
                                                     c5 float, \
...
static void test_fetch_double() {
  ...
  rc = mysql_query(mysql,
                   "CREATE TABLE test_bind_fetch(c1 double, "
                   "c2 double, c3 double, "
                   "c4 double, c5 double, "
                   "c6 double, c7 double)");
  myquery(rc);
...
static void test_field_names() {
  ...
  rc = mysql_query(mysql, "SELECT id as id_alias FROM test_field_names1");
  myquery(rc);
  ...
  /* with table name included with TRUE column name */
  rc = mysql_query(mysql,
                   "SELECT t1.id as id_alias, test_field_names2.name FROM "
...
static void test_warnings() {
  ...
  // kunlun-server doesn't produce warning if 'if exists' added.
  rc = mysql_query(mysql, "DROP TABLE if exists test_non_exists");
  myquery(rc);

  mysql_query(mysql, "DROP TABLE test_non_exists");
...
static void test_insert() {
  ...
  rc = mysql_query(mysql,
                   "CREATE TABLE test_prep_insert(col1 smallint, \
                                col2 varchar(50))");
  myquery(rc);
...
static void test_field_flags() {
  ...
  rc = mysql_query(
      mysql,
  "create index test_field_flags_id3_id4 on test_field_flags(id3, id4)");
  myquery(rc);
...
static void test_stmt_close() {
  ...
  my_stpcpy(query, "select 1");//"DO \"nothing\"");
...
static void test_prepare_alter() {
  ...
  */
  ...
  if (thread_query("ALTER TABLE test_prep_alter rename column id to id_new "))
    exit(1);
  if (thread_query("ALTER TABLE test_prep_alter alter column id_new type varchar(20)"))
    exit(1);
...
CLIENT_MULTI_STATEMENTS is NOT checked anywhere in mysql, and CLIENT_MULTI_RESULTS is
only required when executing stored procedure in mysql. also, CLIENT_MULTI_RESULTS is
by default set in client lib (probably not set in very old client lib?) so there is no
real need to check this flag bit in kunlun-server.
...
static void test_date() {
  ...
  rc = mysql_query(mysql,
                   "CREATE TABLE test_date(c1 TIMESTAMPtz, \
                                                 c2 TIME, \
                                                 c3 timestamp, \
                                                 c4 DATE)");
...
static void test_date_frac() {
  ...
  rc = mysql_query(mysql,
                   "CREATE TABLE test_date(c1 TIMESTAMPtz(6), \
                                                 c2 TIME(6), \
                                                 c3 timestamp(6), \
                                                 c4 DATE)");
...
static void test_fetch_nobuffs() {
  ...
  while (mysql_stmt_fetch(stmt) != MYSQL_NO_DATA) {
    rc++;
    if (!opt_silent) {
      fprintf(stdout, "\n CURRENT_USER    : %s", str[0]);
      fprintf(stdout, "\n CURRENT_DATE    : %s", str[1]);
      fprintf(stdout, "\n CURRENT_TIMESTAMP: %s", str[2]);
...
static void test_ushort_bug() {
  ...
  rc = mysql_query(mysql,
                   "CREATE TABLE test_ushort(a smallint, \
                                                  b smallint, \
                                                  c smallint, \
                                                  d smallint)");
  myquery(rc);
  ...
  if (!opt_silent) {
    ...
    fprintf(stdout, "\n longlong : %s (%ld)", llstr(longlong_value, llbuf),
            (long)ll_length);
    fprintf(stdout, "\n smallint  : %d   (%ld)", tiny_value, (long)t_length);
  }
...
static void test_sshort_bug() {
  ...
  rc = mysql_query(mysql,
                   "CREATE TABLE test_sshort(a smallint , \
                                                  b smallint , \
                                                  c smallint, \
                                                  d smallint)");
  myquery(rc);
...
static void test_stiny_bug() {
  ...
  rc = mysql_query(mysql,
                   "CREATE TABLE test_stiny(a smallint , \
                                                  b smallint , \
                                                  c smallint, \
                                                  d smallint)");
  myquery(rc);
...
static void test_set_option() {
  ...
  rc = mysql_query(mysql, "CREATE TABLE test_limit(a smallint)");
  myquery(rc);
...
static void test_prepare_grant() {
  ...
  rc = mysql_query(
      mysql, "CREATE TABLE test_grant(a smallint primary key auto_increment)");
  myquery(rc);
...
static void test_decimal_bug() {
  ...
  rc = mysql_query(mysql,
                   "insert into test_decimal_bug values(8), (10.22), (5.61)");
  myquery(rc);
...
static void test_truncated_rows() {
  ...
  rc = mysql_query(mysql, "CREATE TABLE t1(c1 smallint)");
  myquery(rc);
  ...
  rc = mysql_query(mysql, "set session sql_mode='TRADITIONAL'");
  myquery(rc);
  rc = mysql_query(mysql, "INSERT INTO t2 SELECT * FROM t1");
  /*
    this test case was wrong, even mysql itself returns error because of the NULL fields.
	so here we should expect rc to be true.
  */
  //myquery_r(rc);
  if (rc == 0)
  {
    rc = mysql_query(mysql, "show session variables like 'sql_mode'");
    result = mysql_store_result(mysql);
    mytest(result);
	
    rc = mysql_query(mysql, "SELECT * FROM t2");
    result = mysql_store_result(mysql);
    mytest(result);
  ...
  DIE_UNLESS(count == 0);// was: DIE_UNLESS(count == 1);  error packet has no place to pass the warning count, even connecting to mysql here returns 0 rather than 1.
  ...
  count = mysql_warning_count(mysql);
  if (!opt_silent) fprintf(stdout, "\n total warnings: %d", count);
  DIE_UNLESS(count == 0); // was 2, kunlun-server is more strict on data types, it returns error for above insert stmt, and as above the error packet won't pass warning count
...
static void test_logs() {
  ...
  rc = mysql_query(mysql,
                   "CREATE TABLE test_logs(id smallint, name varchar(30))");
  myquery(rc);
...
static void test_nstmts() {
  ...
  */
  ...
  if (!opt_silent) fprintf(stdout, "\n 14 prepares each with 100 executions took : %llu usecs, 500 pairs of prepare&execution ops took %llu usecs",
		  timeval_diff(&ts2, &ts1), timeval_diff(&ts3, &ts2));
...
static void test_free_result() {
  ...
  rc = mysql_query(mysql, "insert into test_free_result values(default), (default), (default)");
  myquery(rc);
...
static void test_ts() {
  ...
  rc = mysql_query(mysql, "set timezone='gmt'");
  myquery(rc);
  stmt = mysql_simple_prepare(mysql,
  ...
  */
  ...
  verify_col_data("test_ts", "c", "2003-07-12 21:07:46+00"); // kunlun-server (and pg) returns timestamp values with timezone
...
static void test_bug1500() {
  ...
  */
  ...
  rc = mysql_query(
      mysql,
      "CREATE TABLE test_bg1500 (s VARCHAR(25)) ");
  myquery(rc);
  ...
  stmt = mysql_simple_prepare(
      mysql, "SELECT s FROM test_bg1500 WHERE (s) ~ (?)");
  check_stmt(stmt);
  ...
  stmt = mysql_simple_prepare(mysql,
                              "SELECT s FROM test_bg1500 WHERE (s) "
                              "~ (CONCAT(?, 'digger'))");
  check_stmt(stmt);
...
static void test_subqueries() {
  ...
  rc = mysql_query(mysql, "create table t2 (like t1 including all)"); 
  myquery(rc);
  rc = mysql_query(mysql, "insert into t2 select * from t1;");
...
static void test_distinct() {
  ...
  int rc, i;
  const char *query =
      "SELECT 2+count(distinct b), sum(a) FROM t1 group by a";// group_concat not exist in kunlun-server
...
static void test_bug2248() {
  MYSQL_STMT *stmt;
  int rc;
  const char *query1 = "SELECT current_timestamp";
...
static void test_create_drop() {
  ...
  query = "create table t1 (like t2 including all)";
  ...
  // kunlun-server doesn't support 'create table t1 select a from t2' grammar,
  // we have to split it apart into a CREATE... LIKE stmt and a INSERT INTO... SELECT stmt
  ...
  // drop t1 after preparing above query otherwise the prepare fails
  rc = mysql_query(mysql, "DROP TABLE t1");
  myquery(rc);
  ...
  for (i = 0; i < 3; i++) {
    ...
    rc = mysql_stmt_execute(stmt_create);
    check_execute(stmt_create, rc);
    if (!opt_silent) fprintf(stdout, "created %i again\n", i);

    rc = mysql_stmt_execute(stmt_create_query_block);
    check_execute(stmt_create, rc);
    if (!opt_silent) fprintf(stdout, "insert select %i\n", i);
...
static void test_xjoin() {
  ...
  rc = mysql_query(mysql,
                   "create table t3 (id int, param1_id int, param2_id "
                   "int) ");
  myquery(rc);

  rc = mysql_query(mysql,
                   "create table t1 ( id int, name_id int, value "
                   "varchar(10)) ");
  myquery(rc);

  rc = mysql_query(mysql,
                   "create table t2 (id int, name_id int, value "
                   "varchar(10)) ;");
  myquery(rc);

  rc = mysql_query(mysql,
                   "create table t4(id int, value varchar(10)) ");
  myquery(rc);
...
static void test_bug3035() {
  ...
  /* mins and maxes 
	take int8 as int16 because kunlun-server doesn't support tinyint
  */
  const short int8_min = -32768;//128;
  const short int8_max = 32767;
  const unsigned short uint8_min = 0;
  const unsigned short uint8_max = 65535;
  ...
  bind_array[0].buffer_type = MYSQL_TYPE_SHORT;
  bind_array[0].buffer = (void *)&int8_val;

  bind_array[1].buffer_type = MYSQL_TYPE_SHORT;
...
static void test_view() {
  ...
  rc = mysql_query(mysql, "DROP TABLE IF EXISTS t1,t2,t3 cascade");
  myquery(rc);
  ...
  rc = mysql_query(mysql,
  /*
  kunlun-server doesn't support this grammar for now, join condition must be together with joining tables.

                   "CREATE VIEW v1 AS select "
                   " T0001.SERVERNAME AS SERVERNAME, T0003.TABNAME AS"
                   " TABNAME,T0003.LOCAL_NAME AS LOCAL_NAME,T0002.DBINSTANCE AS"
                   " DBINSTANCE from t2 T0001 join t1 T0002 join t3 T0003 on" // where
                   " ((T0002.SERVERGRP = T0001.SERVERGRP) and"
                   " (T0002.SERVERGRP = T0003.SERVERGRP)"
                   " and (T0003.MAPSTATE = 'A') and"
                   " (T0003.ACTSTATE = ' '))");
*/

"CREATE VIEW v1 AS select "
	"T0001.SERVERNAME AS SERVERNAME,"
	"T0003.TABNAME AS   TABNAME,"
	"T0003.LOCAL_NAME AS LOCAL_NAME,"
	"T0002.DBINSTANCE AS   DBINSTANCE from "
"t2 T0001 join t1 T0002 on (T0002.SERVERGRP = T0001.SERVERGRP) "
"join t3 T0003 on   (T0002.SERVERGRP = T0003.SERVERGRP) "
"where (T0003.MAPSTATE = 'A') and   (T0003.ACTSTATE = ' ') " );
...
static void test_view_where() {
  ...
  rc = mysql_query(mysql, "DROP VIEW IF EXISTS v2");
  myquery(rc);
  rc = mysql_query(mysql, "DROP VIEW IF EXISTS v1");
  myquery(rc);
  ...
  myquery(rc);
  rc = mysql_query(mysql, "DROP TABLE t1 cascade");
  myquery(rc);
}
...
static void test_view_2where() {
  ...
  rc = mysql_query(mysql, "DROP TABLE IF EXISTS LTDX cascade");
  myquery(rc);
  ...
  rc = mysql_query(mysql,
                   ...
                   " TYPE char(1) NOT NULL, SRTF2 int NOT NULL,"
                   ...
                   " INACTIVE char(1) NOT NULL, CLUSTR smallint NOT NULL,"
                   " CLUSTD blob,"
...
static void test_view_insert_fields() {
  ...
  rc =
      mysql_query(mysql,
                  ...
                  "F1C4 varchar(4) NOT NULL, F2I4 int NOT NULL,"
                  "F3N5 varchar(5) NOT NULL default '00000',"
                  "F4I4 int NOT NULL default '0', F5C8 varchar(8) NOT NULL,"
...
static void test_bug6096() {
  ...
  stmt_text =
      "create table t1 (c_smallint smallint, c_smallint smallint, "
      " c_mediumint int, c_int int, "
...
static void test_truncation() {
  ...
  /* unsigned tiny -> tiny both stored as int2 so no error */
  DIE_UNLESS(my_bind++ < bind_array + bind_count);
  ...
  /* ushort -> utiny utinystored as int2 so no error */
  DIE_UNLESS(my_bind++ < bind_array + bind_count);
...
static void test_bug11909() {
  ...
  stmt_text =
      "create table t1 ("
      "  empno int not null, firstname varchar(20) not null,"
...
static void test_bug11901() {
  ...
  stmt_text =
      ...
      " mgrno int not null, location varchar(20) not null,"
      " admrdept varchar(6) not null, refcntd int not null,"
      " refcntu int not null, primary key (deptno))";
  rc = mysql_real_query(mysql, stmt_text, (ulong)strlen(stmt_text));
  myquery(rc);
...
static void test_bug14210() {
  ...
  /*
    ...
    is harmless.
  */
  mysql_query(mysql, "create table t1 (a varchar(255)) ");
...
static void test_bug15613() {
  ...
  rc = mysql_query(mysql,
                   ...
                   "tt text character set utf8, "
                   "mt text character set utf8, "
                   "lt text character set utf8, "
...
static void test_bug14169()
{
  ...
  myquery(rc);
  rc= mysql_query(mysql, "create table t1 (f1 int, f2 varchar(250))");
  myquery(rc);
...
/*
   ...
   this can be enabled when ticket #760 is finished.
*/
static void test_mysql_insert_id() {
  ...
  rc = mysql_query(mysql, "create table t1 (f1 int, f2 varchar(25))");
  myquery(rc);
  rc = mysql_query(mysql, "create index t1_f1_idx on t1(f1)");
  myquery(rc);
  ...
  */
  ...
  rc = mysql_query(
      mysql,
      "create table t2 (f1 int not null primary key "
      "auto_increment, f2 varchar(25), unique (f2)) ");
  myquery(rc);
  ...
  */
  ...
  DIE_UNLESS(res == 15);
#if 0 
  // kunlun-server doesn't support user defined vars
  rc = mysql_query(mysql,
...
static void test_wl4166_1() {
  ...
  rc = mysql_query(mysql,
                   "CREATE TABLE table_4166(col1 smallint NOT NULL, "
                   "col2 varchar(15), col3 int, "
                   "col4 smallint, col5 bigint, "
                   "col6 float, col7 double, "
  ...
  /* smallint */
  my_bind[0].buffer_type = MYSQL_TYPE_SHORT;
  my_bind[0].buffer = (void *)&tiny_sdata;
  /* string */
  my_bind[1].buffer_type = MYSQL_TYPE_STRING;
  my_bind[1].buffer = (void *)str_data;
  my_bind[1].buffer_length = 1000; /* Max string length */
...
static void test_wl5928() {
  ...
  stmt = mysql_simple_prepare(mysql, "CREATE TABLE t1 (f1 INT) ");
...
static void test_bug19894382() {
  ...
  rc = mysql_query(mysql,
                   ...
                   " f2 TIME, f3 TIMESTAMPtz NULL, f4 TIMESTAMP,"
                   " f5 TIME(3), f6 TIMESTAMPtz(3) NULL,"
                   " f7 TIMESTAMP(3));");
  myquery(rc);
...
static void test_param_integer() {
  ...
  const char *str_decimal = "123";// was 1.23 kunlun-server doesn't allow non-integer consts
  const char *str_float = "123";// was 1.23e0 kunlun-server doesn't allow non-integer consts
  ...
  memset(bind_array, 0, sizeof(bind_array));
/*
there is no meaning to support converting a date/time/datetime/timestamp
value to numerical values, because mysql does so simply by
removing all non-digit chars from the input string to get a sequence of digits
as the final integer value, this seems totally non-sense, and logically doing
so just don't make sense and doing so won't bring any convinience at all.
e.g. '2008-09-12 14:23:00' is converted to 20080912142300
'16:28:45' is converted to 162845
'2022-05-30' is converted to 20220530000000
  ...
  mysql_stmt_close(stmt);
*/memset(&dt, 0, sizeof(dt));//kill warning
  stmt_text = "DROP TABLE t1";
  rc = mysql_real_query(mysql, stmt_text, (ulong)strlen(stmt_text));
  myquery(rc);
}
...
static void test_wl12542() {
  ...
  for (int is_null_flag = 0; is_null_flag < 2; is_null_flag++) {
    ...
    wl12542_test_numeric_type(MYSQL_TYPE_SHORT, unsigned short, 12, atoi,
                              is_null_arg);
...
/*
  ...
  ENABLE_BUG_TESTS may be enabled to run the cases for bugs and WLs, but such tests
  are not run yet, there might be failures. in order to avoid unused-functions warning(error),
  we have to put such test cases in my_unused_tests array but don't use them.
*/
static struct my_tests_st my_tests[] = {
   ...
   // {"test_date_dt", test_date_dt},
    ...
    //{"test_ps_i18n", test_ps_i18n}, unsupported grammar
#ifdef ENABLE_BUG_TESTS
...
// ENABLE_BUG_TESTS are listed here.
static struct my_tests_st my_bug_wl_tests[] = {
    {"test_bug1115", test_bug1115},
    {"test_bug1180", test_bug1180},
    {"test_bug1500", test_bug1500},
    {"test_bug1644", test_bug1644},
	...
	*/
    ...
    //{"test_bug32847269", test_bug32847269},
    {nullptr, nullptr}};
struct my_tests_st *get_my_bug_wl_tests() { return my_bug_wl_tests; }
